import { g as generateCspDigest, s as spreadAttributes, u as unescapeHTML, b as renderTemplate, e as removeBase, a as isRemotePath, f as unflatten, o as object, h as date, j as array, A as AstroError, U as UnknownContentCollectionError, k as string } from './ssr-function_0tbWWBgZ.mjs';
import { c as createComponent } from './astro-component_QHW-LdW4.mjs';
import 'clsx';
import { V as VALID_INPUT_FORMATS } from './_astro_assets_imNEL9Uw.mjs';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_B7lu02ec.mjs');
...
function formatMonthYear(date, onlyYear) {
  if (typeof date === "string") {
    const [month2, year2] = date.split("/").map(Number);
    if (Number.isNaN(month2) || Number.isNaN(year2)) {
      return date;
    }
    const monthName = new Date(year2, month2 - 1).toLocaleString("en-US", {
      month: "long"
    });
    if (onlyYear) return String(year2);
    return `${monthName} ${year2}`;
  }
...
export { formatMonthYear as f, getCollection as g };
